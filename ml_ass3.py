...
Original file is located at
    https://colab.research.google.com/drive/10aQhLcIbaXUx-oCjEMS5NhviMaLVcKS3
...
In order to train the model accurately, we need to collect the relevant data. There should be two parts of data for the whole model. The first part should include the posts related to a certain city(Used as the keyword) on Facebook, and then date that it was posted. The second part should include the time(Most likely calculated on a monthly basis)and the tourism revenue(Used as the index of tourism trend) of that month.
In order to focus on one certain city, the example city chosen here is Shanghai.
The first part can be collected using web crawler tools, setting the keyword as the name of the city, which is in this case, Shanghai. The overall time period should be as long as possible to have a more convincing and reliable model.
To collect the tourism revenue data, we can easily get it from relevant tourism websites(eg. 962020.com), the time period should also be as long as possible to make the time series more accurate and stable.

####2.Choose and deploy the classifier of sentiment analysis

Compare classifiers of sentiment analysis and choose the most appropriate one. Use the classifier(eg. TextBlob) to calculate the score of the sentiment. Posts with scores above 0 are positive posts, scores below 0 are negative posts and scores exactly 0 are neutral posts. Record the score of each in the first data file.
...
When the model is built, it should validated before the actual deployment. The model should do the self-prediction and the future prediction test in real life. We can use the model to predict the tourism trend of next two months, and see if the predicted trend is accurate enough. 
...
However, if the accuracy is not good enough, the model should be further improved, get more training data or optimize the data analysis model, so that the reliability of the model can be guaranteed, and its value is further improved.
EXTRA. Future improvement 
Since the time constraint is one year, there are works can’t be done within time schedule, but worth being done in the future. 
That is the real-time prediction, in the future, if we can develop a real-time web crawler system, we will be able to monitor the social sentiment continuously. So that we can do the real-time tourism trend prediction, which can greatly help the relevant managing and marketing.
...
![](https://drive.google.com/uc?id=1Ll-bd9dKIcCAc5a5y1D9sLy0J2qdSHTP)
![](https://drive.google.com/uc?id=1rLu9RMa6vPpms-_mnC1DivGd-Iy85GjW)![](https://drive.google.com/uc?id=1fPY_MqlSygkWXgO5AfQYk4VaHJx1JWF9)
...
The expected outcome is that we can accurately predict the future trend of the tourism, so that we can have a better management over the local tourism.
The model can adapt to data of different cities, so that is can be widely used. The investor can use this predicting model to help them supervise over the local tourism. They can easily know whether their current managing and marketing strategy is good enough or need further improvement. When the current strategy is modified, the system can also help to predict the possible outcome of the modified strategy. Also, this system can help the investor know the current social sentiments toward this city, and how these sentiment will affect the tourism, so that the investor can react instantly according to the prediction.
Overall, this project can help the investor to manage the tourism more easily and effectively, so that their tourism revenue can be improved steady.
...
Software includes purchasing web crawler system, and other professional data analyzing tools.
The approximate software budget will be 800$.

###Hardware

Hardware includes renting an office, purchasing PCs and relevant office tools.
The approximate hardware budget will be: 30000 Office Renting + 800(PC) x 10 +2500(Office tools)=40500$

Human resource

Human resource include the salary of the project team: 8500(per person per month) x 16 x 12 =1632000$

Total: So the total budget is approximately 800(Software)+40500(Hardware)+1632000(Human resource)= 1673300
So that the proposed budget should be around 1700000 $.

##5)PERSONNE
The project team is consist of project manager, primary data analysts, senior data analysts, system testers and accountant.
One project manager will be required to supervise and control the whole project, also he will be in charge of communicating with investors.
Four system testers will be required to test the methods, including the sentiment analysis methods, model building methods and the overall forecasting system.
Five primary data analysts will be required to collect useful data, three of them will be collecting the Facebook posts data, and two of them will be collecting the monthly tourism revenue data. Also the primary data analysts will help to build the model.
Five senior data analysts will be required to build ,deploy and improve the data analysis model.
One accountant will be required to record all the costs of the project.
